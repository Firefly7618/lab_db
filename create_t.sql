...
CREATE TABLE users (
    ...
    email VARCHAR2(30) NOT NULL UNIQUE CONSTRAINT check_mail CHECK(REGEXP_LIKE(email, '^(\S+)\@(\S+)\.(\S+)$')),
...
ALTER TABLE Albums ADD CONSTRAINT unique_albums UNIQUE(title, singer_id);
...
ALTER TABLE Songs ADD CONSTRAINT unique_songs UNIQUE(title, singer_id);
